...
// Source file has been modified as follows:
// - Changed namespace and added preprocessor statements
// - Added progress bar feedback

namespace Hirabiki.Extern.VRLabs.AV3Manager
{
    public static class AnimatorCloner
    {
        ...
        public static AnimatorController MergeControllers(AnimatorController mainController, AnimatorController controllerToMerge, Dictionary<string, string> paramNameSwap = null, bool saveToNew = false)
        {
            ...
            for(int i = 0; i < controllerToMerge.layers.Length; i++)
            {
                ...
                float p = Mathf.Lerp(0.90f, 1.00f, (float)i / controllerToMerge.layers.Length);
                EditorUtility.DisplayProgressBar("VRCLens Setup", $"Combining user and VRCLens animators - Layer {i}", p);
                // -- END --
            ...
            // Additional progress bar feedback
            EditorUtility.DisplayProgressBar("VRCLens Setup", "Refreshing assets", 1.0f);
            // -- END --
