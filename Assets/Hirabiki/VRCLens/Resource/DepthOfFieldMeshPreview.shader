	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v){
				...
				o.uv = v.uv;
#else
				// is 60-degree -- player or VRChat camera viewport
				bool isCam = abs(unity_CameraProjection._m11 - 1.73205) < 0.00001
					&& unity_OrthoParams.w == 0.0 && _ProjectionParams.y <= 0.05 && all(unity_CameraProjection._m20_m21 == 0.0) // AND is Perspective AND Far-Plane <= 0.05
		...
		Pass
		{
			...
			half4 dither(half2 uv, half4 c) {
				...
				half3 rand = hash33(half3(uv * _ScreenParams.xy, frac(_Time.y) * 256.0)) * 0.004 - 0.002;
