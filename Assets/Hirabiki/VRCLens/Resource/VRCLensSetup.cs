// VRCLens Copyright (c) 2020-2021 Hirabiki. All rights reserved.
...
namespace Hirabiki.AV3.Works.VRCLens
{
    ...
    public class VRCLensSetup : MonoBehaviour
    {
        ...
        public int focalIndex = 2, apertureIndex = 2, sensorSizeIndex = 0, msaaIndex = 1;
        ...
        public bool IsSetupCompatible()
        {
            ...
            Transform testHead = Avatar.GetBoneTransform(HumanBodyBones.Head);
            Transform testLeftHand = Avatar.GetBoneTransform(HumanBodyBones.LeftHand);
            Transform testRightHand = Avatar.GetBoneTransform(HumanBodyBones.RightHand);

            return testHead != null && testLeftHand != null && testRightHand != null;
        }
        ...
        public void AppendAnimationSetup()
        {
            ...
            if(skipAppendParam)
            {
                AssetDatabase.CopyAsset("Assets/VRCSDK/Examples3/Expressions Menu/DefaultExpressionParameters.asset", $"{_userCopyFolderPath}/VRCLensParams_Instance.asset");
            ...
            if(AssetDatabase.GetAssetPath(userMenu) == $"{PrefabPath}/Menus2/VRCL2_Base.asset")
            {
                ...
                // Things has changed now - ALWAYS copy over the menu, even for the overwrite setup
                AssetDatabase.CopyAsset("Assets/VRCSDK/Examples3/Expressions Menu/DefaultExpressionsMenu.asset", $"{_userCopyFolderPath}/VRCLensBaseMenu_Instance.asset");
        ...
        public void Configurate()
        {
            ...
            _avatarRoot = avatarDescriptor.transform;
            _head = Avatar.GetBoneTransform(HumanBodyBones.Head);
            _leftHand = Avatar.GetBoneTransform(setupMode == SetupMode.VRRightHand ? HumanBodyBones.LeftHand : HumanBodyBones.RightHand);
            _rightHand = Avatar.GetBoneTransform(setupMode == SetupMode.VRRightHand ? HumanBodyBones.RightHand : HumanBodyBones.LeftHand);
            
            // [1]
            pickupA.SetParent(_rightHand);
            pickupA.localPosition = Vector3.zero;
            pickupA.localRotation = Quaternion.identity;
            pickupA.localScale = Vector3.one;

            pickupB.SetParent(_leftHand);
            pickupB.localPosition = Vector3.zero;
            pickupB.localRotation = Quaternion.identity;
            pickupB.localScale = Vector3.one;

            pickupC.SetParent(_head);
            pickupC.localPosition = Vector3.zero;
            pickupC.localRotation = Quaternion.identity;
            pickupC.localScale = Vector3.one;

            vrcLensRoot.SetParent(_avatarRoot);
            ...
            */
            ...
            ActivateScaleConstraint(pickupC.GetComponent<ScaleConstraint>(), _head.parent);
            ...
            camScreen.SetPositionAndRotation(_head.position, _head.rotation);
            ...
            ActivateParentConstraint(auxCopy.GetComponent<ParentConstraint>(), _head);
        ...
        private void ModifyStateDefaults(AnimatorController anim)
        {
            ...
            ModifyAnimationClipDefaults(state_HeadScaleLocal.motion as AnimationClip, _head.Find("PickupC"));
            ModifyAnimationClipDefaults(state_HeadScaleRemote.motion as AnimationClip, _head.Find("PickupC"));
            ...
            if(animLegacyStream)
            {
                if(animDirectStream) sm_DirectStream.defaultState = state_DSEnabled_Old;
                else sm_DirectStream.defaultState = state_DSDisabled_Old;
            } else
            {
                if(animDirectStream) sm_DirectStream.defaultState = state_DSEnabled;
                else sm_DirectStream.defaultState = state_DSDisabled;
            }
            ...
            VRCAvatarParameterDriver paramDriver = (VRCAvatarParameterDriver)state_InitRadials.behaviours[0];
