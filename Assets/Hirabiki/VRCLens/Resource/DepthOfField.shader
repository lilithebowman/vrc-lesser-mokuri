	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				isCam *= unity_OrthoParams.w == 0.0 && _ProjectionParams.y <= 0.05;
...
#undef NEAR_PLANE
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				bool isRender = (isWindowed && all(_ScreenParams.xy == half2(1920, 1080) || _ScreenParams.xy == _RenderTex_TexelSize.zw))
					|| (!isWindowed && (any(_ScreenParams.xy != half2(1024, 512)) || _IsDirectStream) );
...
#define RENDER_RAD_SCALE 4.000000000
				...
				for (int i = 0; i < finalCount; i++) {
					...
					half2 tc = texCoord + scaleSampling(diskKernel[i]) * radiusScale;
#else
					...
					//float radius = lengthOctagon(diskKernel[i]) * radiusScale;
		...
		Pass
		{
			...
			*/
			...
			half4 drawMiddleDisplay(half4 src, half2 iuv) {
				half2 uv = half2((iuv.x - 0.5) * 1.777777777 + 0.5, iuv.y);
				...
				col.a = uv.y < 0.125 ? col.a : _FocusDistance > 0.5001 ? col.a : 0.0;
			...
			half2 shiftSymbols(half2 uv) {
				...
				shift.x = uv.x * 1.333333333 < 0.125
				  ? y < 1 ? +0
				  : y < 2 ? +0
				  : y < 3 ? _SensorType - 1
				  : y < 4 ? +0
				  ...
				  : +0 : 0.0;
				shift.x = (1 - uv.x) * 1.333333333 < 0.125
				  ? y < 1 ? -0
				  : y < 2 ? -0
				  : y < 3 ? -0
				  : y < 4 ? -0
			...
			half4 maskSymbols(half2 uv) {
				...
				c.a = uv.x < 0.5
				  ? y < 1 ? _EnableDoF
				  : y < 2 ? (frac(_Time.y * 0.666666666) < 0.666666666) * _IsDirectStream
				  : y < 3 ? _SensorType != 0
...
#define LOGICALXOR(p, q) ((p || q) && !(p && q))
			half4 frag(v2f i) : SV_Target
			{
				...
				// ---- FOCUS PEAKING ---- //
				half offsetA0 = cos(UNITY_PI * (uv.y - uv.x + _Time.y * 0.2) * 128.0) * -0.5 + 0.5;
				half offsetA1 = step(frac((uv.y - uv.x + _Time.y*0.2) * 32.0), 0.5);
				col = isPreview && _EnableDoF && LOGICALXOR(_ShowFocusPeeking, !isAutoFocus)
					? lerp(col, half4(half3(1.0,0.0,0.0), 1.0), edgeDetect(rawColor.rgb, sbsUV0, finalExp.x, smoothstep(1.0, 0.5, abs(rawColor.a)) * 10.0, 0.02)) : col;
				// ---- Exposure Zebra ---- //
				half offsetB = saturate(cos(UNITY_PI * (uv.y + uv.x + _Time.y*0.2) * 64.0) * 2.0);
				col = isPreview && _ShowZebra ? lerp(col, half4(0,0,0,1),
					offsetB * step(3.0, min(1.5,col.r) + min(1.5,col.g) + min(1.5,col.b))) : col;
				// ---- Rule of Thirds grid ---- //
				...
				symUV.x = uv.x < 0.5 ? symUV.x : symUV.x - 0.333333333;
				...
				col = isPreview ? printNumber(_FocalLength * _SensorScale, 4, 0, col, (uv - half2(0.770,-0.001)) * 0.88 * half2(8.0, 9.0)) : col;
				col = isPreview && _EnableDoF ? printNumber(realAperture, 3, 1, col, (uv - half2(0.045,-0.001)) * 0.88 * half2(9.14285714, 9.0)) : col;
