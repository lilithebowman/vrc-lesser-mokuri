...
namespace Hirabiki.AV3.Works.VRCLens
{
    ...
    public class ConfigInspector : Editor
    {
        ...
        readonly string[] tonemapOptions = { "Off (RAW)", "Filmic 1 (ACES)", "Filmic 2 (EVILS) *", "HDR (Hybrid Log-Gamma)", "Neutral (Reinhard)" };
...
デスクトップモードでプレイしたら、緑のカメラがあるところに頭に取り付いています。"));
            ...
            if(up != int.MaxValue)
            {
                EditorGUILayout.LabelField(new GUIContent($"Expression Parameter Usage: {rp * 0.125f} ({(config.AV3_MAXPARAMALLOC - up - rp) * 0.125f} remaining)",
                    @"Avatars 3.0 allows for the maximum of 16 expressions parameter in an avatar.
It is recommended to not use more expression parameters on VRCLens than needed.
Note: Bool-type parameter uses 0.125 rather of 1"));
            ...
            config.tonemapIndex = EditorGUILayout.Popup(new GUIContent("Picture Style", "Tone mapping (HDR) presets\n* Recommended"), config.tonemapIndex, tonemapOptions);
...
The selected avatar has a setup that is incompatible with VRCLens. Please contact hirabiki on Twitter to help make every avatar compatible.", MessageType.Error);
            ...
            {
                EditorGUILayout.HelpBox("Not enough free parameters space to add.", MessageType.Error);
                except = true;
